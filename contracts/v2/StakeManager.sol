...
contract StakeManager is Governable, ReentrancyGuard, TaxAcknowledgement, Pausable {
    ...
    /// @param _employerSlashPct Percentage or basis points of the slashed amount sent to employer.
    ///        Accepts integers in the range 0-100 (interpreted as percentages) or 0-10_000 (basis points).
    /// @param _treasurySlashPct Percentage or basis points of the slashed amount sent to treasury.
    ///        Accepts integers in the range 0-100 (interpreted as percentages) or 0-10_000 (basis points).
    ...
    constructor(
        ...
        uint256 employerSlashBps = _normalizeSlashPct(_employerSlashPct);
        uint256 treasurySlashBps = _normalizeSlashPct(_treasurySlashPct);
        if (employerSlashBps + treasurySlashBps == 0) {
            employerSlashPct = 0;
            treasurySlashPct = SLASH_BPS_DENOMINATOR;
        } else {
            if (employerSlashBps + treasurySlashBps != SLASH_BPS_DENOMINATOR) {
                revert InvalidPercentage();
            }
            employerSlashPct = employerSlashBps;
            treasurySlashPct = treasurySlashBps;
        }
        burnSlashPct = 0;
    ...
    function _normalizeSlashPct(uint256 value) private pure returns (uint256) {
        if (value == 0) {
            return 0;
        }
        if (value <= 100) {
        ...
        if (value <= SLASH_BPS_DENOMINATOR) {
            return value;
        }
        revert InvalidPercentage();
    }
    ...
    function _toBps(uint256 value) private pure returns (uint16) {
        uint256 normalized = _normalizeSlashPct(value);
        if (normalized > SLASH_BPS_DENOMINATOR) revert InvalidPercentage();
    ...
    /// @param _employerSlashPct Percentage (0-100) or basis points (0-10_000) sent to employer
    /// @param _treasurySlashPct Percentage (0-100) or basis points (0-10_000) sent to treasury
    ...
    /// @param _validatorSlashPct Percentage (0-100) or basis points (0-10_000) of the total slashed amount distributed to validators
    ...
    /// @param _employerSlashPct Percentage (0-100) or basis points (0-10_000) sent to the employer
    /// @param _treasurySlashPct Percentage (0-100) or basis points (0-10_000) sent to the treasury
    /// @param _validatorSlashPct Percentage (0-100) or basis points (0-10_000) sent to validators
    ...
    /// @param _operatorSlashPct Percentage (0-100) or basis points (0-10_000) of the total slashed amount added to the operator reward pool
    ...
    /// @param _employerSlashPct Percentage (0-100) or basis points (0-10_000) sent to the employer
    /// @param _treasurySlashPct Percentage (0-100) or basis points (0-10_000) sent to the treasury
    /// @param _operatorSlashPct Percentage (0-100) or basis points (0-10_000) sent to the operator reward pool
    /// @param _validatorSlashPct Percentage (0-100) or basis points (0-10_000) sent to validators
