...
export class PlanetaryOrchestrator {
  ...
  async applyOwnerCommand(command: OwnerCommand): Promise<FabricEvent[]> {
    ...
    switch (command.type) {
      ...
      case 'job.cancel': {
        const located = this.resolveJobCommandTarget(command, {
  ...
  private resolveJobCommandTarget(
    command: { jobId?: string; locator?: JobLocator },
    options: { allowCompleted: boolean; allowFailed: boolean }
  ): LocatedJob {
    const { jobId, locator } = command;
    const located = jobId ? this.findJob(jobId) : undefined;
    const resolved = located ?? (locator ? this.locateJob(locator) : undefined);
    if (!resolved) {
      if (jobId) {
        throw new Error(`Unknown job ${jobId}`);
      }
      throw new Error('Owner command could not locate a job matching the provided criteria');
    }
    if (!options.allowCompleted && resolved.location === 'completed') {
      throw new Error(`Job ${resolved.job.id} is already completed and cannot be modified`);
    }
    if (!options.allowFailed && resolved.location === 'failed') {
      throw new Error(`Job ${resolved.job.id} is already failed and cannot be modified`);
    }
    return resolved;
  }
  ...
  private locateJob(locator: JobLocator): LocatedJob | undefined {
    switch (locator.kind) {
      case 'tail': {
        const shardIds = locator.shard ? [locator.shard] : Array.from(this.shards.keys());
        const eligible: LocatedJob[] = [];
        for (const shardId of shardIds) {
          const shard = this.shards.get(shardId);
          if (!shard) {
            continue;
          }
          for (let index = shard.queue.length - 1; index >= 0; index -= 1) {
            eligible.push({ job: shard.queue[index], shardId, shard, location: 'queue', queueIndex: index });
          }
          if (locator.includeInFlight) {
            for (const job of shard.inFlight.values()) {
              eligible.push({ job, shardId, shard, location: 'inFlight', node: job.assignedNodeId ? this.nodes.get(job.assignedNodeId) : undefined });
            }
          }
        }
        ...
        const index = Math.min(offset, eligible.length - 1);
