"""Run the Meta-Agentic α-AGI Jobs Demo V8 orchestrator."""
...
from meta_agentic_alpha_v8 import DEFAULT_CONFIG_PATH, run_meta_convergence_demo  # noqa: E402  pylint: disable=wrong-import-position


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Meta-Agentic α-AGI Jobs Demo V8")
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_CONFIG_PATH,
        help="Path to the V8 scenario YAML (defaults to the bundled scenario)",
    )
    parser.add_argument(
        "--timeout",
        type=float,
    ...
    parser.add_argument(
        "--timeout-env",
        default=os.environ.get("META_AGENTIC_DEMO_V8_TIMEOUT"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    outcome = run_meta_convergence_demo(args.config, timeout=timeout)

    metrics = outcome.dashboard_payload["metrics"]
    control_surface = outcome.dashboard_payload["control_surface"]

    print("\n🎖️ Meta-Agentic α-AGI Jobs Demo V8 completed!")
