...
const DATA_PATH = "dashboard-data-v6.json";
...
function renderSovereignty(target, surface) {
  ...
  list.innerHTML = `
    ...
    <li>Session keys ${surface.session_keys.join(", ")}</li>
  `;
  target.appendChild(list);
...
async function bootstrap() {
  ...
  document.getElementById("alpha-compounding").textContent = `${(data.metrics.alpha_compounding_index * 100).toFixed(1)}%`;
}
...
bootstrap().catch((error) => {
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    `<div class="error">Unable to load dashboard-data-v6.json. Re-run meta_agentic_demo_v6.py and refresh.</div>`
  );
});
