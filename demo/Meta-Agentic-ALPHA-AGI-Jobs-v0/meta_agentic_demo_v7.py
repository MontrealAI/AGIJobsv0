"""Run the Meta-Agentic α-AGI Jobs Demo V7 orchestrator."""
...
from meta_agentic_alpha_v7 import DEFAULT_CONFIG_PATH, run_meta_singularity_demo  # noqa: E402  pylint: disable=wrong-import-position


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Meta-Agentic α-AGI Jobs Demo V7")
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_CONFIG_PATH,
        help="Path to the V7 scenario YAML (defaults to the bundled scenario)",
    )
    parser.add_argument(
        "--timeout",
        type=float,
    ...
    parser.add_argument(
        "--timeout-env",
        default=os.environ.get("META_AGENTIC_DEMO_V7_TIMEOUT"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    outcome = run_meta_singularity_demo(args.config, timeout=timeout)

    metrics = outcome.dashboard_payload["metrics"]
    control_surface = outcome.dashboard_payload["control_surface"]

    print("\n🎖️ Meta-Agentic α-AGI Jobs Demo V7 completed!")
    print(f"   • Orchestration run ID: {outcome.base.run_id}")
    print(f"   • Run state: {outcome.base.status.run.state}")
    print(
        "   • Alpha compounding index: "
        f"{metrics['alpha_compounding_index']*100:.2f}%"
    )
    print(
        "   • Owner empowerment: "
        f"{metrics['owner_empowerment']*100:.2f}%"
    )
    print(
        "   • Unstoppable readiness: "
        f"{metrics['unstoppable_readiness']*100:.2f}% (CI status {outcome.dashboard_payload['ci']['status']})"
    )
    print(
