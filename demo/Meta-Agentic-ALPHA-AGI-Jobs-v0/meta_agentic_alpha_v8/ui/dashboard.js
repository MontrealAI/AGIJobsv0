...
const DATA_PATH = "dashboard-data-v8.json";
...
async function bootstrap() {
  ...
  renderMermaid(document.getElementById("mermaid-quadrant"), data.mermaid.quadrant);
  ...
  document.getElementById("capital-flywheel").textContent = `${(data.metrics.capital_flywheel_index * 100).toFixed(1)}%`;
  document.getElementById("convergence").textContent = `${(data.metrics.convergence_velocity * 100).toFixed(1)}%`;
  document.getElementById("guardian-resilience").textContent = `${(data.metrics.guardian_resilience * 100).toFixed(1)}%`;
}

bootstrap().catch((error) => {
  console.error("Failed to bootstrap Meta-Convergence console", error);
});
