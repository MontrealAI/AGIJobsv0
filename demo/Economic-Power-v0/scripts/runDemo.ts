...
const layer2PlaybookSchema = z.object({
  id: z.string(),
  name: z.string(),
  script: z.string(),
  objective: z.string(),
...
const layer2ReadinessFactorSchema = z.object({
  id: z.string(),
  label: z.string(),
...
const layer2Schema = z.object({
  network: z.string(),
  chainId: z.number(),
  rollupType: z.enum(['optimistic', 'zk']),
  status: z.enum(['pilot', 'ready', 'expansion']),
  readinessScore: z.number().min(0).max(1),
  settlementTimeMinutes: z.number().nonnegative(),
  bridge: z.object({
    contract: z.string(),
    ownerSafe: z.string(),
    upgradeScript: z.string(),
    watchers: z.array(z.string()),
    monitoringCadenceMinutes: z.number().positive(),
  }),
  sequencer: z.object({
    operator: z.string(),
    backupOperators: z.array(z.string()),
    decentralizationPlan: z.string(),
  }),
  monitoring: z.object({
    dashboards: z.array(z.string()),
    alerts: z.array(z.string()),
  }),
  readinessFactors: z.array(layer2ReadinessFactorSchema),
  guardrails: z.array(z.string()),
  playbooks: z.array(layer2PlaybookSchema),
  commentary: z.string(),
});
...
type Layer2Config = z.infer<typeof layer2Schema>;
type Layer2ReadinessFactor = z.infer<typeof layer2ReadinessFactorSchema>;
type Layer2Playbook = z.infer<typeof layer2PlaybookSchema>;
...
type Layer2ReadinessClassification =
  | 'transcendent-dominion'
  | 'mission-ready'
  | 'in-flight'
  | 'attention';

type Layer2ReadinessFactorSummary = {
  id: string;
  label: string;
  status: Layer2ReadinessFactor['status'];
  weight: number;
  contribution: number;
  description: string;
};

type Layer2ReadinessReport = {
  network: string;
  chainId: number;
  rollupType: Layer2Config['rollupType'];
  status: Layer2Config['status'];
  readinessScore: number;
  classification: Layer2ReadinessClassification;
  settlementTimeMinutes: number;
  settlementSummary: string;
  bridge: Layer2Config['bridge'];
  sequencer: Layer2Config['sequencer'];
  monitoring: Layer2Config['monitoring'];
  guardrails: string[];
  playbooks: Layer2Playbook[];
  readinessFactors: Layer2ReadinessFactorSummary[];
  commentary: string;
  recommendedActions: string[];
  mermaid: string;
};
...
function generateLayer2ReadinessMarkdown(report: Layer2ReadinessReport): string {
  const lines: string[] = [];
  lines.push('# Layer-2 Readiness Dossier');
  lines.push('');
  lines.push(
    `Network **${report.network}** (chainId ${report.chainId}) — ${report.rollupType.toUpperCase()} rollup in ${report.status} stage.`,
  );
  lines.push(
    `Readiness score ${(report.readinessScore * 100).toFixed(1)}% (${report.classification.replace(/-/g, ' ')}).`,
  );
  lines.push(report.settlementSummary);
  lines.push('');
  lines.push('## Bridge control');
  lines.push(`- Contract: \`${report.bridge.contract}\``);
  lines.push(`- Owner safe: \`${report.bridge.ownerSafe}\``);
  lines.push(`- Upgrade script: \`${report.bridge.upgradeScript}\``);
  lines.push(`- Monitoring cadence: every ${report.bridge.monitoringCadenceMinutes} minutes`);
  lines.push('');
  lines.push('## Watchers & monitoring');
  const watcherList = report.bridge.watchers.length ? report.bridge.watchers : ['No watchers configured'];
  for (const watcher of watcherList) {
    lines.push(`- Watcher: ${watcher}`);
  }
  const dashboards = report.monitoring.dashboards.length
    ? report.monitoring.dashboards
  ...
  const alerts = report.monitoring.alerts.length ? report.monitoring.alerts : ['No alert channels configured'];
  lines.push('');
  lines.push('### Alerts');
  ...
  lines.push('');
  lines.push('## Sequencer programme');
  lines.push(`- Operator: ${report.sequencer.operator}`);
  lines.push(
    `- Backups: ${report.sequencer.backupOperators.length ? report.sequencer.backupOperators.join(', ') : 'None'}`,
  );
  lines.push(`- Decentralisation plan: ${report.sequencer.decentralizationPlan}`);
  lines.push('');
  lines.push('## Guardrails');
  if (report.guardrails.length === 0) {
    lines.push('- No guardrails defined. Add guardrail programmes immediately.');
  } else {
  ...
  if (report.playbooks.length === 0) {
    lines.push('- No playbooks encoded.');
  } else {
    for (const playbook of report.playbooks) {
      lines.push(`- **${playbook.name}** — \`${playbook.script}\` (${playbook.objective})`);
    }
  }
  lines.push('');
  lines.push('## Readiness factors');
  lines.push('| Factor | Weight | Contribution | Status | Description |');
  lines.push('| --- | --- | --- | --- | --- |');
  for (const factor of report.readinessFactors) {
    lines.push(
      `| ${factor.label.replace(/\|/g, '\\|')} | ${(factor.weight * 100).toFixed(1)}% | ${(factor.contribution * 100).toFixed(1)}% | ${factor.status.toUpperCase()} | ${factor.description.replace(/\|/g, '\\|')} |`,
    );
  }
  lines.push('');
  ...
  lines.push('');
  lines.push(`_Commentary:_ ${report.commentary}`);
  lines.push('');
  lines.push('_Generated by AGI Jobs Economic Power orchestration._');
  lines.push('');
  return `${lines.join('\n')}\n`;
...
function buildOwnerAutopilot(summary: Summary, scenario: Scenario): OwnerAutopilot {
  ...
  if (summary.layer2Readiness) {
    guardrails.push(
      `Layer-2 readiness ${(summary.layer2Readiness.readinessScore * 100).toFixed(1)}% • ${summary.layer2Readiness.settlementSummary}`,
    );
  }
  ...
  if (summary.layer2Readiness?.playbooks.length) {
    const playbook = summary.layer2Readiness.playbooks[0];
    sequence.unshift({
      programId: playbook.id,
  ...
  return {
    ...
    telemetry: {
      ...
      layer2ReadinessScore: summary.metrics.layer2ReadinessScore,
    },
...
function classifyLayer2Readiness(
  score: number,
  status: Layer2Config['status'],
): { classification: Layer2ReadinessClassification; summary: string } {
  if (score >= 0.95) {
    return {
      classification: 'transcendent-dominion',
      summary:
        'Layer-2 execution mesh operates with unstoppable supremacy – bridge, sequencer, and guardrails are rehearsed and fully scripted for owner dominance.',
    };
  }
  if (score >= 0.85) {
    return {
      classification: 'mission-ready',
      summary:
        `Layer-2 deployment is mission ready – ${status} stage with hardened guardrails and deterministic bridge coverage awaiting full-scale ignition.`,
    };
  }
  if (score >= 0.7) {
    return {
      ...
      summary:
        'Layer-2 program is in-flight – continue executing rehearsals, audits, and guardrail upgrades to reach unstoppable status.',
    };
  }
  return {
    classification: 'attention',
    summary:
      'Layer-2 readiness requires immediate attention – activate playbooks and guardrails to prevent control gaps before expansion.',
  };
}

function buildLayer2ReadinessReport(layer2: Layer2Config): Layer2ReadinessReport {
  const totalWeight = layer2.readinessFactors.reduce((acc, factor) => acc + Math.max(factor.weight, 0), 0) || 1;
  const readinessFactors: Layer2ReadinessFactorSummary[] = layer2.readinessFactors.map((factor) => {
    const normalisedWeight = factor.weight <= 0 ? 0 : factor.weight / totalWeight;
    const statusScore = factor.status === 'complete' ? 1 : factor.status === 'in-progress' ? 0.6 : 0.2;
    return {
      id: factor.id,
      label: factor.label,
      status: factor.status,
      weight: Number(normalisedWeight.toFixed(3)),
      contribution: Number((normalisedWeight * statusScore).toFixed(3)),
      description: factor.description,
    };
  });
  const computedScore = readinessFactors.reduce((acc, factor) => acc + factor.contribution, 0);
  const aggregatedScore = clamp01((layer2.readinessScore + computedScore) / 2);
  const { classification, summary } = classifyLayer2Readiness(aggregatedScore, layer2.status);
  const recommendedActions: string[] = [];
  for (const factor of readinessFactors) {
    if (factor.status === 'in-progress') {
      recommendedActions.push(`Complete readiness factor: ${factor.label}.`);
    } else if (factor.status === 'blocked') {
      recommendedActions.push(`Unblock readiness factor: ${factor.label}.`);
    }
  }
  if (layer2.bridge.watchers.length < 3) {
    recommendedActions.push('Add additional bridge watchers to reach triple-sign anomaly coverage.');
  }
  if (layer2.sequencer.backupOperators.length < 2) {
    recommendedActions.push('Register additional sequencer backup operators to guarantee zero downtime.');
  }
  if (layer2.guardrails.length === 0) {
    recommendedActions.push('Author guardrail catalog for layer-2 emergency actions.');
  }
  if (recommendedActions.length === 0) {
    recommendedActions.push('Maintain guardrail rehearsal cadence and continue on-chain attestations for the bridge.');
  }
  const watchersLabel = layer2.bridge.watchers.length
    ? layer2.bridge.watchers.join(', ')
    : 'Unassigned watchers';
  const backupLabel = layer2.sequencer.backupOperators.length
    ? layer2.sequencer.backupOperators.join(' | ')
    : 'Pending backup operators';
  const alertLabel = layer2.monitoring.alerts.length
    ? layer2.monitoring.alerts.join(' / ')
    : 'No alert channels';
  const mermaidLines = [
    'graph TD',
    '    Owner[Owner Multi-Sig] -->|Bridge controls| Bridge',
    `    Bridge["Bridge ${layer2.bridge.contract}"] -->|Finalize ${layer2.settlementTimeMinutes}m| Mainnet[(Ethereum Mainnet)]`,
    `    Bridge --> L2[${layer2.network} Rollup]`,
    `    L2 --> Sequencer["Sequencer ${layer2.sequencer.operator}"]`,
    `    Sequencer --> Backups[Backups ${backupLabel}]`,
    `    Bridge --> Watchers[Watchers ${watchersLabel}]`,
    `    Watchers --> Alerts[Alerts ${alertLabel}]`,
    `    Alerts --> Owner`,
    `    Owner --> Playbooks[Playbooks ${layer2.playbooks.length}]`,
    `    Owner --> Guardrails[Guardrails ${layer2.guardrails.length}]`,
  ];
  return {
    network: layer2.network,
    chainId: layer2.chainId,
    rollupType: layer2.rollupType,
    status: layer2.status,
    readinessScore: Number(aggregatedScore.toFixed(3)),
    classification,
    settlementTimeMinutes: layer2.settlementTimeMinutes,
    settlementSummary: `Settlements confirm on L1 in ${layer2.settlementTimeMinutes} minutes with ${layer2.bridge.watchers.length} watcher(s).`,
    bridge: { ...layer2.bridge },
    sequencer: { ...layer2.sequencer },
    monitoring: { ...layer2.monitoring },
    guardrails: [...layer2.guardrails],
    playbooks: [...layer2.playbooks],
    readinessFactors,
    commentary: layer2.commentary,
    recommendedActions,
...
function buildOwnerDominion(summary: Summary): OwnerDominionReport {
  ...
  if (summary.layer2Readiness) {
    if (summary.layer2Readiness.classification !== 'transcendent-dominion') {
      const action =
    ...
    for (const action of summary.layer2Readiness.recommendedActions) {
      const entry = `Layer-2: ${action}`;
      if (!recommendedActions.includes(entry)) {
        recommendedActions.push(entry);
      }
    }
  }
  if (recommendedActions.length === 0) {
...
function buildSuperIntelligence(summary: Summary): SuperIntelligenceReport {
  ...
  const weights = {
    economicDominance: 0.18,
    ownerSupremacy: 0.18,
    sovereignSafety: 0.16,
    automation: 0.12,
    shockResilience: 0.12,
  ...
  const indexRaw =
    ...
    metrics.layer2ReadinessScore * weights.layer2;
  const index = Number(Math.min(1, Math.max(0, indexRaw)).toFixed(4));
  const { classification, summary: narrative } = classifySuperIntelligence(index);

  const layer2 = summary.layer2Readiness;
  const drivers = [
    ...
    layer2
      ? `Layer-2 readiness ${(layer2.readinessScore * 100).toFixed(1)}% protecting ${layer2.network} with ${layer2.bridge.watchers.length} watcher(s) and ${layer2.playbooks.length} encoded playbooks.`
      : 'Layer-2 readiness pending configuration – encode bridge guardrails to unlock unstoppable expansion.',
  ];
  ...
  const layer2Command = layer2
    ? `Layer-2 ${layer2.classification} with settlement ${layer2.settlementTimeMinutes}m and watchers ${layer2.bridge.watchers.join(', ')} ready for owner execution.`
    : 'Layer-2 program awaiting activation – configure scenario guardrails and playbooks to extend dominion beyond L1.';
  commandAssurance.push(layer2Command);
  ...
  const mermaidLines = [
    ...
    `  Layer2["Layer-2 ${(metrics.layer2ReadinessScore * 100).toFixed(1)}%"] --> Apex`,
  ];
  ...
  return {
    ...
    telemetry: {
      ...
      layer2ReadinessScore: metrics.layer2ReadinessScore,
    },
    mermaid: `${mermaidLines.join('\n')}\n`,
  };
}
...
function synthesiseSummary(
  ...
  return {
    ...
    metrics: {
      ...
      layer2ReadinessScore: scenario.layer2
        ? Number(clamp01(scenario.layer2.readinessScore).toFixed(3))
        : 0,
    },
    ownerControl: {
...
function crossValidateSummary(summary: Summary, scenario: Scenario): CrossValidationReport {
  ...
  const metrics: CrossValidationMetricCheck[] = [
    ...
    buildMetricCheck(
      'layer2ReadinessScore',
      'Layer-2 readiness derived from scenario factors',
      scenario.layer2 ? buildLayer2ReadinessReport(scenario.layer2).readinessScore : 0,
...
export async function runScenario(
  ...
  summary.layer2Readiness = workingScenario.layer2
    ? buildLayer2ReadinessReport(workingScenario.layer2)
    : null;
  summary.metrics.layer2ReadinessScore = summary.layer2Readiness
    ? summary.layer2Readiness.readinessScore
    : 0;
...
async function writeOutputs(
  ...
  if (summary.layer2Readiness) {
    await fs.writeFile(
      path.join(outputDir, 'layer2-readiness.json'),
      JSON.stringify(summary.layer2Readiness, null, 2),
    );
    await fs.writeFile(
      path.join(outputDir, 'layer2-readiness.mmd'),
      `${summary.layer2Readiness.mermaid.trimEnd()}\n`,
    );
    await fs.writeFile(
      path.join(outputDir, 'layer2-readiness.md'),
      generateLayer2ReadinessMarkdown(summary.layer2Readiness),
    );
  }
  await fs.writeFile(
