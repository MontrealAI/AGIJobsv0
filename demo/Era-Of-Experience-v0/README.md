...
npm run demo:era-of-experience:triangulate   # multi-seed verification for compounding GMV/ROI lift
...
### Triangulation safety drill

- `npm run demo:era-of-experience:triangulate` executes the loop across multiple deterministic seeds, writes
  `reports/triangulation.json`, and renders `reports/triangulation.mmd` – a consensus mermaid graph demonstrating
  cross-seed agreement.
- Flags any seed that fails to beat baseline GMV, ROI, or autonomy, and calculates a confidence score combining
  dominance ratio and consensus success to assure the operator the system is delivering compounding value from
  every angle.
