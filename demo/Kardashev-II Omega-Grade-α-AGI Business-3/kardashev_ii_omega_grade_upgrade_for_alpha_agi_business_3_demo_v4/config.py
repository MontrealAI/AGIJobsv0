...
class UpgradeV4Paths:
    """Filesystem artefact layout for the Kardashev-II Ω Upgrade v4 demo."""
    ...
    def __init__(self, root: Path) -> None:
        ...
        self.telemetry = root / "status" / "omega-upgrade-v4" / "telemetry.json"
        self.telemetry_ui = root / "status" / "omega-upgrade-v4" / "telemetry-ui.json"
        self.mermaid = root / "status" / "omega-upgrade-v4" / "job-graph.mmd"
        self.long_run_ledger = root / "status" / "omega-upgrade-v4" / "long-run-ledger.jsonl"
        self.guardian_plan = root / "status" / "omega-upgrade-v4" / "autonomy-plan.json"
        self.guardian_history = root / "status" / "omega-upgrade-v4" / "autonomy-history.jsonl"
        self.autonomy_checkpoint = root / "status" / "omega-upgrade-v4" / "autonomy-checkpoint.json"
        self.storyboard = root / "status" / "omega-upgrade-v4" / "storyboard.json"
        self.storyboard_history = root / "status" / "omega-upgrade-v4" / "storyboard-history.jsonl"
        self.insight_journal = root / "status" / "omega-upgrade-v4" / "insights.jsonl"
        self.mission_manifest = root / "status" / "omega-upgrade-v4" / "mission-manifest.json"
        for path in (
...
class OmegaOrchestratorV4Config(OrchestratorConfig):
    """Configuration envelope extended with v4 upgrade artefacts."""
...
def build_config(overrides: Optional[Dict[str, Any]] = None) -> OmegaOrchestratorV4Config:
    """Build an :class:`OmegaOrchestratorV4Config` from overrides."""

    config = OmegaOrchestratorV4Config()
    defaults: Dict[str, Any] = {
        ...
        "telemetry_output_path": Path("artifacts/status/omega-upgrade-v4/telemetry.json"),
        "telemetry_ui_payload_path": Path("artifacts/status/omega-upgrade-v4/telemetry-ui.json"),
        "mermaid_output_path": Path("artifacts/status/omega-upgrade-v4/job-graph.mmd"),
        "long_run_ledger_path": Path("artifacts/status/omega-upgrade-v4/long-run-ledger.jsonl"),
        "guardian_plan_path": Path("artifacts/status/omega-upgrade-v4/autonomy-plan.json"),
        "guardian_history_path": Path("artifacts/status/omega-upgrade-v4/autonomy-history.jsonl"),
        "autonomy_checkpoint_path": Path("artifacts/status/omega-upgrade-v4/autonomy-checkpoint.json"),
        "storyboard_path": Path("artifacts/status/omega-upgrade-v4/storyboard.json"),
        "storyboard_history_path": Path("artifacts/status/omega-upgrade-v4/storyboard-history.jsonl"),
        "insight_journal_path": Path("artifacts/status/omega-upgrade-v4/insights.jsonl"),
        "mission_manifest_path": Path("artifacts/status/omega-upgrade-v4/mission-manifest.json"),
    ...
    if overrides:
        dataclass_fields = {field_info.name for field_info in fields(OmegaOrchestratorV4Config)}
...
def load_config(path: Path, overrides: Optional[Dict[str, Any]] = None) -> OmegaOrchestratorV4Config:
