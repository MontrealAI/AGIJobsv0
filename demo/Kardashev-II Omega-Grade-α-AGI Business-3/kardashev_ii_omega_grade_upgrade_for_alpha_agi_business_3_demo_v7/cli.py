"""Command-line entry point for the Ω-grade upgrade v7 demonstration."""
...
from .config import OmegaOrchestratorV7Config, load_config
from .orchestrator import OmegaUpgradeV7Orchestrator
...
async def _run_orchestrator(config: OmegaOrchestratorV7Config) -> None:
    orchestrator = OmegaUpgradeV7Orchestrator(config)
...
def _owner_stream(config: OmegaOrchestratorV7Config) -> OwnerCommandStream:
...
def _command_launch(args: argparse.Namespace) -> None:
    ...
    if args.mesh_history_lines is not None:
        overrides["mesh_history_lines"] = int(args.mesh_history_lines)
    if args.structured_log_history is not None:
        overrides["structured_log_history_lines"] = int(args.structured_log_history)
    if args.resource_ledger is not None:
        overrides["resource_ledger_path"] = str(args.resource_ledger)
    if args.mesh_history_path is not None:
        overrides["mesh_history_path"] = str(args.mesh_history_path)
    if args.structured_log_path is not None:
        overrides["structured_log_path"] = str(args.structured_log_path)
...
def _command_owner(args: argparse.Namespace) -> None:
    ...
    if args.subcommand == "pause":
        ...
        if args.mesh_history_lines is not None:
            mission_updates["mesh_history_lines"] = int(args.mesh_history_lines)
        if args.structured_log_history is not None:
            mission_updates["structured_log_history_lines"] = int(args.structured_log_history)
...
def _command_ci(_: argparse.Namespace) -> None:
    ...
    overrides = {
        ...
        "mesh_history_lines": 128,
        "structured_log_history_lines": 256,
    }
...
def main(argv: Optional[list[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Operate the Kardashev-II Ω Upgrade v7 planetary orchestration demo",
    )
    parser.set_defaults(func=lambda _: parser.print_help())
    subparsers = parser.add_subparsers(dest="command")

    launch_parser = subparsers.add_parser("launch", help="Start the Ω Upgrade v7 orchestrator loop")
    ...
    launch_parser.add_argument("--mesh-history-lines", type=int, default=None, help="Retain this many mesh bus history entries")
    launch_parser.add_argument("--structured-log-history", type=int, default=None, help="Retain this many structured log entries")
    launch_parser.add_argument("--resource-ledger", type=Path, default=None, help="Override planetary resource ledger path")
    launch_parser.add_argument("--mesh-history-path", type=Path, default=None, help="Override mesh bus history path")
    launch_parser.add_argument("--structured-log-path", type=Path, default=None, help="Override structured log path")
    ...
    mission.add_argument("--mesh-history-lines", type=int, default=None)
    mission.add_argument("--structured-log-history", type=int, default=None)
