...
class UpgradeV7Paths:
    ...
    def __init__(self, root: Path) -> None:
        ...
        self.resource_ledger = root / "status" / "omega-upgrade-v7" / "resource-ledger.json"
        self.mesh_history = root / "status" / "omega-upgrade-v7" / "mesh-history.jsonl"
...
def build_config(overrides: Optional[Dict[str, Any]] = None) -> OmegaOrchestratorV7Config:
    ...
    defaults: Dict[str, Any] = {
        ...
        "resource_ledger_path": Path("artifacts/status/omega-upgrade-v7/resource-ledger.json"),
        "mesh_history_path": Path("artifacts/status/omega-upgrade-v7/mesh-history.jsonl"),
        ...
        "mesh_history_lines": 8192,
        "structured_log_history_lines": 16384,
...
def load_config(path: Path, overrides: Optional[Dict[str, Any]] = None) -> OmegaOrchestratorV7Config:
    ...
    if "paths" in data:
        ...
        cli_overrides.setdefault("resource_ledger_path", artefacts.resource_ledger)
        cli_overrides.setdefault("mesh_history_path", artefacts.mesh_history)
