...
from .bus import Message as MeshMessage, OmegaMessageBus
...
from .resources import PlanetaryResourceLedger
...
class StructuredLogWriter:
    """Append-only JSONL writer with retention control."""
    ...
    def _append(self, payload: Dict[str, Any]) -> None:
        line = json.dumps(payload, default=_serialize_default, separators=(",", ":"))
        if self.path.exists():
            history = [row for row in self.path.read_text(encoding="utf-8").splitlines() if row]
        else:
            history = []
        history.append(line)
...
class MeshRecorder:
    """Persist mesh bus traffic for auditing."""
    ...
    async def record(self, message: MeshMessage) -> None:
        payload = {
...
class OmegaUpgradeV7Orchestrator(OmegaUpgradeOrchestrator):
    ...
    def __init__(self, config: OmegaOrchestratorV7Config) -> None:
        ...
        self.config = config
        self.mesh_bus = OmegaMessageBus()
        self._structured_log = StructuredLogWriter(
            Path(getattr(config, "structured_log_path", Path("structured-log.jsonl"))),
            history_limit=int(getattr(config, "structured_log_history_lines", 16384)),
        )
        self._mesh_recorder = MeshRecorder(
            Path(getattr(config, "mesh_history_path", Path("mesh-history.jsonl"))),
            history_limit=int(getattr(config, "mesh_history_lines", 8192)),
        )
        self.mesh_bus.register_listener(self._mesh_recorder.record)
        self.bus.register_listener(self._bridge_mesh_event)
        old_resources = self.resources
        ledger = PlanetaryResourceLedger(
            energy_capacity=old_resources.energy_capacity,
            compute_capacity=old_resources.compute_capacity,
            base_token_supply=float(getattr(old_resources, "token_supply", 0.0))
            + sum(
                float(getattr(account, "locked", 0.0))
                for account in getattr(old_resources, "_accounts", {}).values()
            ),
            ledger_path=Path(
                getattr(config, "resource_ledger_path", Path("resource-ledger.json"))
            ),
            history_limit=int(getattr(config, "structured_log_history_lines", 16384)),
        )
        ledger.energy_available = float(getattr(old_resources, "energy_available", ledger.energy_available))
        ledger.compute_available = float(getattr(old_resources, "compute_available", ledger.compute_available))
        ledger.token_supply = float(getattr(old_resources, "token_supply", ledger.token_supply))
        ledger.energy_price = float(getattr(old_resources, "energy_price", ledger.energy_price))
        ledger.compute_price = float(getattr(old_resources, "compute_price", ledger.compute_price))
        ledger._accounts = dict(getattr(old_resources, "_accounts", {}))  # type: ignore[attr-defined]
        ledger._reservations = dict(getattr(old_resources, "_reservations", {}))  # type: ignore[attr-defined]
    ...
    async def _bridge_mesh_event(
        self, topic: str, payload: Dict[str, Any], publisher: str
    ) -> None:
        await self.mesh_bus.publish(topic, payload, publisher)
        await self._structured_log.record(
            "bus_event", topic=topic, publisher=publisher, payload=payload
        )
    ...
    async def _resource_feedback_loop(self) -> None:
        try:
            while self._running:
                ...
                await self._structured_log.record(
                    "resource_feedback", **self._governor_state
                )
    ...
    def _build_state_snapshot(self) -> StateSnapshot:
        ...
        topic_stats = self.mesh_bus.topic_stats
        mesh_snapshot = {
            "topics": list(topic_stats.keys()),
            "subscriptions": topic_stats,
        }
        logs_snapshot = {
            "structured_log_path": str(self._structured_log.path),
            "resource_ledger_path": str(getattr(self.resources, "_ledger_path", "")),
            "mesh_history_path": str(self._mesh_recorder.path),
        }
