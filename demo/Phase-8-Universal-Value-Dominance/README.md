...
2. **Generate calldata & telemetry**:
  ...
  * Autogenerated files in `demo/Phase-8-Universal-Value-Dominance/output/`:
     ...
     * `phase8-invariant-audit.md` — independent cross-check of sentinel coverage, capital funding, and guardrail adequacy.
...
3. **Triple-verify guardrails** (deterministic audit report):
   ```bash
   npm run demo:phase8:audit
   ```
   This synthesises `output/phase8-invariant-audit.md`, cross-checking sentinel coverage, domain funding, and autonomy caps via two independent calculation paths. Any mismatch is surfaced in both the console output and the markdown report so operators can correct configuration drift before shipping transactions.
