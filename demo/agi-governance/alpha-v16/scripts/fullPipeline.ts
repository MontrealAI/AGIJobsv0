...
async function main(): Promise<void> {
  ...
  if (hasError) {
    console.error("❌ Full α-field v16 ChronoSovereign pipeline completed with errors.");
    process.exitCode = 1;
  } else if (hasWarning) {
    console.warn("⚠️ Full α-field v16 ChronoSovereign pipeline completed with warnings.");
  } else {
    console.log("✅ Full α-field v16 ChronoSovereign pipeline executed cleanly.");
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to orchestrate α-field v16 ChronoSovereign full pipeline:", error);
    process.exitCode = 1;
  });
}
