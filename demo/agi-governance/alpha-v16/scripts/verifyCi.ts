...
const OUTPUT_FILE = path.join(REPORT_DIR, "ci-verification-v16.json");
...
async function main(): Promise<void> {
  ...
  if (assessment.ok) {
    console.log("✅ v16 mission confirms the CI (v2) enforcement shield.");
  } else {
    console.error("❌ CI shield drift detected for ChronoSovereign mission:");
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to verify CI for α-field v16 ChronoSovereign:", error);
    process.exitCode = 1;
  });
}
