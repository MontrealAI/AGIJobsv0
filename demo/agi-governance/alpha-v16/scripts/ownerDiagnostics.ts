...
const JSON_REPORT = path.join(REPORT_DIR, "owner-diagnostics-v16.json");
const MARKDOWN_REPORT = path.join(REPORT_DIR, "owner-diagnostics-v16.md");
...
async function main(): Promise<void> {
  ...
  console.log("✅ Owner supremacy lattice audited for ChronoSovereign mission.");
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to aggregate owner diagnostics for α-field v16 ChronoSovereign:", error);
    process.exitCode = 1;
  });
}
