...
async function main(): Promise<void> {
  ...
  if (report.totals.failed === 0) {
    console.log("✅ α-field v16 dossier validated by independent recomputation.");
  } else {
    console.warn("⚠️ Validation detected discrepancies — inspect the ChronoSovereign report.");
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to validate α-field v16 ChronoSovereign dossier:", error);
    process.exitCode = 1;
  });
}
