...
async function main(): Promise<void> {
  ...
  if (report.totals.failed === 0) {
    console.log("✅ α-field v16 HyperSovereign dossier validated by independent recomputation.");
  } else {
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to validate α-field v16 HyperSovereign dossier:", error);
    process.exitCode = 1;
  });
}
