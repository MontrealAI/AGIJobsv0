...
const JSON_REPORT = path.join(REPORT_DIR, "owner-diagnostics-v13.json");
const MARKDOWN_REPORT = path.join(REPORT_DIR, "owner-diagnostics-v13.md");
...
async function main(): Promise<void> {
  ...
  console.log("✅ Owner command lattice audited.");
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to aggregate owner diagnostics for α-field v13:", error);
    process.exitCode = 1;
  });
}
