...
const REPORT_FILE = path.join(REPORT_DIR, "governance-demo-report-v13.md");
const SUMMARY_FILE = path.join(REPORT_DIR, "governance-demo-summary-v13.json");
const DASHBOARD_FILE = path.join(REPORT_DIR, "governance-demo-dashboard-v13.html");
...
const CI_REPORT = path.join(REPORT_DIR, "ci-verification-v13.json");
const OWNER_JSON = path.join(REPORT_DIR, "owner-diagnostics-v13.json");
const OWNER_MARKDOWN = path.join(REPORT_DIR, "owner-diagnostics-v13.md");
const FULL_JSON = path.join(REPORT_DIR, "governance-demo-full-run-v13.json");
const FULL_MARKDOWN = path.join(REPORT_DIR, "governance-demo-full-run-v13.md");
...
async function main(): Promise<void> {
  ...
  if (hasError) {
    console.error("❌ Full α-field v13 pipeline completed with errors.");
    process.exitCode = 1;
  } else if (hasWarning) {
    console.warn("⚠️ Full α-field v13 pipeline completed with warnings.");
  } else {
    console.log("✅ Full α-field v13 pipeline executed cleanly.");
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to orchestrate α-field v13 full pipeline:", error);
    process.exitCode = 1;
  });
}
