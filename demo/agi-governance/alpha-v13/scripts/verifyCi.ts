...
const OUTPUT_FILE = path.join(REPORT_DIR, "ci-verification-v13.json");
...
async function main(): Promise<void> {
  ...
  if (assessment.ok) {
    console.log("✅ v13 mission confirms the CI (v2) enforcement shield.");
  } else {
    console.error("❌ CI shield drift detected:");
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to verify CI for α-field v13:", error);
    process.exitCode = 1;
  });
}
