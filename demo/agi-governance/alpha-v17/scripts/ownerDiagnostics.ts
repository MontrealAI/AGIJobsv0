...
const JSON_REPORT = path.join(REPORT_DIR, "owner-diagnostics-v17.json");
const MARKDOWN_REPORT = path.join(REPORT_DIR, "owner-diagnostics-v17.md");
...
async function main(): Promise<void> {
  ...
  console.log("✅ Owner supremacy lattice audited for Singularity Dominion mission.");
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to aggregate owner diagnostics for α-field v17 Singularity Dominion:", error);
    process.exitCode = 1;
  });
}
