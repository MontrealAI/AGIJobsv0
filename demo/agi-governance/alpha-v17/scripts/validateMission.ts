...
async function main(): Promise<void> {
  ...
  if (report.totals.failed === 0) {
    console.log("✅ α-field v17 dossier validated by independent recomputation.");
  } else {
    console.warn("⚠️ Validation detected discrepancies — inspect the Singularity Dominion report.");
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to validate α-field v17 Singularity Dominion dossier:", error);
    process.exitCode = 1;
  });
}
