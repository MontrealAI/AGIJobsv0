...
const VALIDATION_JSON = path.join(REPORT_DIR, "governance-demo-validation-v14.json");
const VALIDATION_MARKDOWN = path.join(REPORT_DIR, "governance-demo-validation-v14.md");
const CI_REPORT = path.join(REPORT_DIR, "ci-verification-v14.json");
const OWNER_JSON = path.join(REPORT_DIR, "owner-diagnostics-v14.json");
const OWNER_MARKDOWN = path.join(REPORT_DIR, "owner-diagnostics-v14.md");
const FULL_JSON = path.join(REPORT_DIR, "governance-demo-full-run-v14.json");
const FULL_MARKDOWN = path.join(REPORT_DIR, "governance-demo-full-run-v14.md");
...
async function main(): Promise<void> {
  ...
  if (hasError) {
    console.error("❌ Full α-field v14 HyperSovereign pipeline completed with errors.");
    process.exitCode = 1;
  } else if (hasWarning) {
    console.warn("⚠️ Full α-field v14 HyperSovereign pipeline completed with warnings.");
  } else {
    console.log("✅ Full α-field v14 HyperSovereign pipeline executed cleanly.");
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to orchestrate α-field v14 HyperSovereign full pipeline:", error);
    process.exitCode = 1;
  });
}
