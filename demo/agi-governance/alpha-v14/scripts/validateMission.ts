...
async function main(): Promise<void> {
  ...
  if (report.totals.failed === 0) {
    console.log("✅ α-field v14 dossier validated by independent recomputation.");
  } else {
    console.warn("⚠️ Validation detected discrepancies — inspect the HyperSovereign report.");
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error("❌ Failed to validate α-field v14 HyperSovereign dossier:", error);
    process.exitCode = 1;
  });
}
