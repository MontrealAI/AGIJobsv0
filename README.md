...
### Validator Workflow
  ...
  - **Random validator selection** – the contract owner can replace the entire validator list with [`setValidatorPool`](contracts/AGIJobManagerv1.sol#L1662-L1679); each job draws validators pseudo‑randomly from this pool, skipping blacklisted or under-qualified addresses to mitigate race conditions and collusion. Frontends can query `isSelectedValidator(jobId, address)` to confirm selection.
