...
- **Supply-chain provenance:** The `apps-images` workflow now emits signed SLSA Level 3 provenance for both console and portal containers, using job-level permissions tuned for the generator. Release captains can download the provenance bundle from the run summary to verify image digests against the branch rule manifest before rolling out upgrades.【F:.github/workflows/apps-images.yml†L1-L115】
