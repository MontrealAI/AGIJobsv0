...
StakeManager and FeePool constructors each accept an `IERC20 token` address and an optional `_treasury`. `StakeManager` now also takes optional `jobRegistry` and `disputeModule` addresses so a deployer can wire core modules in the same transaction. Deployments here default to the $AGIALPHA token above, and passing `address(0)` uses the owner as treasury. Should economics change, the owner may later call `setToken` on these modules to point to a different ERC‑20 without redeploying. If a zero token address is supplied, both modules automatically fall back to the $AGIALPHA default.
