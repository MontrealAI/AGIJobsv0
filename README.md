...
- **Coordinated pausing:** [`SystemPause`](contracts/v2/SystemPause.sol) centralises pauser delegation across the job registry, stake manager, dispute system, reputation engine, and more. Owners can rotate pausers, pause every module with `pauseAll`, and restore operations with `unpauseAll` in a single transaction, guaranteeing rapid response during contingencies.【F:contracts/v2/SystemPause.sol†L16-L377】
