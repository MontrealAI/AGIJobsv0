...
6. Finalize with `validateJob` or `disapproveJob` after the review window; correct validators split the reserved reward and the configured share of any slashed stakes. Monitor `ValidationCommitted`, `ValidationRevealed`, and `JobFinalizedAndBurned` events.
...
| `slashedStakeRecipient` | `setSlashedStakeRecipient(address)` | Recipient of slashed stake not redistributed to validators |
| `slashedStakeRedistributionPercentage` | `setSlashedStakeRedistributionPercentage(uint256)` | Portion of slashed stake awarded to correct validators |
...
- If no validator votes correctly, slashed stake goes to `slashedStakeRecipient` and the reserved validator reward portion returns to the job's agent or employer; if at least one validator is correct, they split the reserved reward plus the configured share of slashed stake, with any remainder sent to `slashedStakeRecipient`.
...
- **Stake-based validator incentives**
  ...
  - Misaligned votes are slashed and lose reputation; correct validators split the configured share of slashed stake.
  ...
  - If no validator votes correctly, all slashed stakes are sent to `slashedStakeRecipient` and the reserved reward portion refunds to the agent or employer; otherwise, correct validators share the reserved reward plus the configured portion of slashed stake.
...
- **Configurable slashed stake recipient & redistribution** – the owner controls `slashedStakeRecipient` (initially the deployer) and `slashedStakeRedistributionPercentage`. If no validator votes correctly, all slashed stake is sent to `slashedStakeRecipient` and the validator reward portion reverts to the agent or employer. When at least one validator is correct, they split the reserved reward plus the configured share of slashed stake; any remainder still goes to `slashedStakeRecipient`.
...
3. Ensure each validator has staked at least `stakeRequirement` before validating and each agent meets the greater of `agentStakeRequirement` or `payout * agentStakePercentage / 10_000` and `minAgentReputation` before applying.
...
- **Quick-start:**
 ...
 4. **Approve or disapprove** – once the review window elapses, finalize with `validateJob` or `disapproveJob`.
  ...
  5. **Rewards & slashing** – when required approvals/disapprovals are met, correct validators split `validationRewardPercentage` of escrow plus the configured share of slashed stake. Incorrect votes lose `validatorSlashingPercentage` of their bonded tokens.
...
- **Owner‑configurable parameters:** [setValidatorConfig](contracts/AGIJobManagerv1.sol#L1515-L1574), [setStakeRequirement](contracts/AGIJobManagerv1.sol#L1414-L1416), [setValidatorSlashingPercentage](contracts/AGIJobManagerv1.sol#L1428-L1435), [setAgentSlashingPercentage](contracts/AGIJobManagerv1.sol#L1439-L1443), [setValidationRewardPercentage](contracts/AGIJobManagerv1.sol#L1360-L1365), [setMinValidatorReputation](contracts/AGIJobManagerv1.sol#L1445-L1448), [setSlashedStakeRecipient](contracts/AGIJobManagerv1.sol#L1406-L1410), and [setSlashedStakeRedistributionPercentage](contracts/AGIJobManagerv1.sol#L1411-L1422).
- Validators must maintain an on-chain stake and reputation before voting. `stakeRequirement` defines the minimum bonded $AGI, while `validatorSlashingPercentage` dictates how much of that stake is forfeited on an incorrect vote. When a job concludes, validators whose votes match the outcome split `validationRewardPercentage` of the remaining escrow plus the configured share of slashed stake; others lose the slashed amount. The owner may set `validationRewardPercentage` to `0` to disable rewards entirely.
...
- **Aligned rewards** – when a job finalizes, only validators whose votes match the outcome split `validationRewardPercentage` basis points of the remaining escrow along with the configured share of slashed stake. If no votes are correct, slashed tokens go to `slashedStakeRecipient` and the reserved validator reward portion is returned to the job's agent or employer.
...
- **Remainder handling** – integer division leftovers from reward calculations are distributed across correct validators. If no validator votes correctly, all slashed stake goes to `slashedStakeRecipient` and the validator reward pool returns to the agent or employer as appropriate; otherwise, any undistributed remainder of slashed stake also goes to `slashedStakeRecipient`.
...
- Disapproving validators split `validationRewardPercentage` basis points of the escrow along with the configured share of slashed stake and any rounding remainder. If none disapprove correctly, slashed tokens go to `slashedStakeRecipient` and the reward portion returns to the employer.
...
await agiJobManager.connect(v3).disapproveJob(jobId, "", []); // employer wins, v2 & v3 split rewards and configured share of slashed stake
...
- `setSlashedStakeRedistributionPercentage(uint256 percentage)` – set the portion of slashed stake shared with correct validators (basis points); emits `SlashedStakeRedistributionPercentageUpdated`.
