...
| Foundry toolchain | v1.6.0 | `foundry-rs/foundry-toolchain@v1` steps in every workflow explicitly request `v1.6.0`; `ci:verify-toolchain` asserts the pin | Guarantees that `forge`/`anvil`/`cast` behave consistently across CI, fuzzing, and release pipelines. |
