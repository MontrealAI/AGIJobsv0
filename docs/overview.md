...
| StakeManager     | `setMinStake` (`0`), `setSlashingPercentages` (`0`, `100`), `setTreasury` (0 or DAO-controlled), `setJobRegistry` (0 address), `setDisputeModule` (0 address), `setMaxStakePerAddress` (`0`) | Adjust staking token, minimums, slashing rules, and authorised modules.                           |
