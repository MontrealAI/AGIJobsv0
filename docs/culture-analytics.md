...
1. **Indexer SQLite database** (`culture-graph.db`) for artifacts, influence scores, and
   citation counts.
2. **Arena orchestrator API** (`/arena/scoreboard`) for live round telemetry.
3. **Structured logs** (`stake-manager` logs and moderation audit trail) for slash,
   streak, and safety insights.
...
Or run continuously (default hourly) with:
...
| `CULTURE_ORCHESTRATOR_LOG` | Stake manager structured log path | `demo/CULTURE-v0/logs/orchestrator.jsonl` |
