...
1. Install Node.js 20.19.0 and enable npm caching via `actions/setup-node`, matching the
   pipeline toolchain. Export `CYPRESS_INSTALL_BINARY=https://github.com/cypress-io/cypress/releases/download/v13.17.0/cypress-linux-x64.zip`
   to reproduce the workflow’s deterministic Cypress mirror when you run `npm ci` locally.
