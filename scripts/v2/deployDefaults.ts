...
async function main() {
  ...
  const disputeContract = await ethers.getContractAt('contracts/v2/modules/DisputeModule.sol:DisputeModule', disputeModule);
  const committee = await disputeContract.committee(); // address of ArbitratorCommittee
  ...
  await verify(validationModule, [
    ...
    0, 0, // _disputeFee, _disputeWindow (defaults in ValidationModule)
  ...
  console.log('  AttestationRegistry:', identityRegistry); // (AttestationRegistry not used in v2, identityRegistry covers identity)
