...
const UI_ROOT = resolve(__dirname, '../../demo/agi-governance/ui');
...
async function runSimulation(): Promise<void> {
  ...
  console.log('\n🚀 Launching Solving α-AGI Governance grand demo...');
  try {
    await spawnHardhat();
...
async function startServer(): Promise<void> {
  ...
  console.log('\n💡 Commands:');
  console.log('   • Press Enter (or type "run") to replay the entire governance scenario.');
...
function attachCommandInterface(): void {
  ...
  process.stdin.on('data', async (chunk) => {
    ...
    try {
      ...
      console.error('Replay failed. Resolve the issue and run again.');
    }
  });
}
...
(async () => {
  try {
    ...
    console.error('Failed to start control room:', error);
    await shutdown();
    process.exit(1);
  }
})();
