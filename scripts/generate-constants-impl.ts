...
function normaliseWholePercentage(
  ...
  if (!/^\d+$/.test(input)) {
    throw new Error(`${label} must be a whole number of percentage points`);
  }
  const wholePoints = Number.parseInt(input, 10);
  if (!Number.isFinite(wholePoints) || Number.isNaN(wholePoints)) {
    throw new Error(`${label} must be an integer`);
  }
  if (wholePoints <= 0 || wholePoints > 100) {
    throw new Error(`${label} must be within 1-100 (whole points)`);
  }
  const decimal = wholePoints / 100;
  const basisPoints = wholePoints * 100;
  return {
    input,
    decimal,
    percent: wholePoints,
...
function normaliseWindowSeconds(
  raw: string | undefined,
  label: string,
  fallback: string
) {
...
export async function main() {
  ...
  const feePct = normaliseWholePercentage(process.env.FEE_PCT, 'FEE_PCT', '2');

  const burnPct = normaliseWholePercentage(
  ...
  const commitWindow = normaliseWindowSeconds(
    process.env.COMMIT_WINDOW_S,
    'COMMIT_WINDOW_S',
    '1800'
  );
  const revealWindow = normaliseWindowSeconds(
