...
function normalisePercentage(
  ...
  const input = (raw ?? String(fallbackPoints)).trim();
  if (!input) {
    throw new Error(`${label} is required`);
  }
  if (!/^\d+$/.test(input)) {
    throw new Error(`${label} must be a whole-number percentage (e.g. "2" for 2%)`);
  }
...
function normaliseDurationSeconds(
  raw: string | undefined,
  label: string,
  fallbackSeconds: number
) {
  const input = (raw ?? String(fallbackSeconds)).trim();
  if (!input) {
    throw new Error(`${label} is required`);
  }
  if (!/^\d+$/.test(input)) {
...
export async function main() {
  ...
  const feePct = normalisePercentage(process.env.FEE_PCT, 'FEE_PCT', 2);

  const burnPct = normalisePercentage(process.env.BURN_PCT, 'BURN_PCT', 6);
