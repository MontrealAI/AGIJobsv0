...
if [ "$current_npm_version" != "$corepack_npm_version" ]; then
  if npm install -g --no-progress --loglevel=error "npm@${corepack_npm_version}" >/dev/null 2>&1; then
    current_npm_version="$($npm_exec -v 2>/dev/null || printf '')"
  else
    echo "[docker-npm-ci] WARNING: npm install -g npm@${corepack_npm_version} failed; continuing with npm ${current_npm_version:-unknown}" >&2
  fi
fi

if [ "$current_npm_version" != "$corepack_npm_version" ]; then
  echo "[docker-npm-ci] WARNING: Unable to activate npm@${corepack_npm_version}; continuing with npm ${current_npm_version:-unknown}" >&2
fi
