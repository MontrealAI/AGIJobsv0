...
# Force npm to respect package-locks even if upstream images export conflicting
# defaults. We create a transient npmrc that pins package-lock=true and redirect
# both env variants so the setting wins regardless of caller configuration.
temp_npmrc="$(mktemp)"
...
# Surface the resolved package-lock config to simplify diagnosing EUSAGE errors
# when container base images inject conflicting npm defaults.
lock_setting="unknown"
if [ -n "$npm_prepare_args" ]; then
  if output=$(cd "$project_root" && $npm_exec $npm_prepare_args config get package-lock 2>/dev/null); then
    lock_setting="$output"
  fi
else
  if output=$(cd "$project_root" && $npm_exec config get package-lock 2>/dev/null); then
    lock_setting="$output"
  fi
fi
echo "[docker-npm-ci] npm config package-lock=${lock_setting}" >&2
