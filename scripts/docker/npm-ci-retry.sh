...
target_npm_version="${NPM_CI_NPM_VERSION:-}"
current_npm_version="$(npm -v 2>/dev/null || printf '')"

npm_exec="npm"
npm_prepare_args=""

if [ -n "$target_npm_version" ]; then
  if command -v corepack >/dev/null 2>&1; then
    if corepack enable >/dev/null 2>&1; then
      if corepack prepare "npm@${target_npm_version}" --activate >/dev/null 2>&1; then
        current_npm_version="$(npm -v 2>/dev/null || printf '')"
      else
        echo "[docker-npm-ci] WARNING: corepack prepare npm@${target_npm_version} failed; continuing with npm ${current_npm_version:-unknown}" >&2
      fi
    else
      echo "[docker-npm-ci] WARNING: Failed to enable corepack; continuing with npm ${current_npm_version:-unknown}" >&2
    fi
  fi

  if [ "$current_npm_version" != "$target_npm_version" ]; then
    if command -v npx >/dev/null 2>&1; then
      echo "[docker-npm-ci] activating npm@${target_npm_version} via npx fallback" >&2
      npm_exec="npx"
      npm_prepare_args="--yes npm@${target_npm_version}"
      current_npm_version="$($npm_exec $npm_prepare_args -v 2>/dev/null || printf '')"
    else
      echo "[docker-npm-ci] WARNING: Unable to locate npx for npm@${target_npm_version}; continuing with npm ${current_npm_version:-unknown}" >&2
    fi
  fi
fi
