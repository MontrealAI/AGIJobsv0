...
target_npm_version="${NPM_CI_NPM_VERSION:-11.4.2}"
...
if command -v corepack >/dev/null 2>&1; then
  if [ -n "$current_npm_version" ] && [ "$current_npm_version" != "$target_npm_version" ]; then
    if corepack enable >/dev/null 2>&1; then
      if corepack prepare "npm@${target_npm_version}" --activate >/dev/null 2>&1; then
        current_npm_version="$(npm -v 2>/dev/null || printf '')"
      else
        echo "[docker-npm-ci] WARNING: corepack prepare npm@${target_npm_version} failed; continuing with npm $current_npm_version" >&2
      fi
    else
...
if [ -n "$current_npm_version" ] && [ -f "$version_check_script" ]; then
  if ! node "$version_check_script" "$current_npm_version" "$target_npm_version"; then
    if command -v npx >/dev/null 2>&1; then
      echo "[docker-npm-ci] upgrading npm via npx npm@${target_npm_version}" >&2
      npm_exec_cmd="npx"
      npm_exec_args="--yes npm@${target_npm_version}"
      fallback_version="$(sh -c "$npm_exec_cmd $npm_exec_args -v" 2>/dev/null || printf '%s' "$current_npm_version")"
      current_npm_version="$fallback_version"
    else
      echo "[docker-npm-ci] ERROR: npm $current_npm_version is older than required $target_npm_version and npx is unavailable" >&2
      exit 1
    fi
  fi
fi
...
display_npm_version="$(sh -c "$npm_exec_cmd $npm_exec_args -v" 2>/dev/null || printf 'unknown')"
echo "[docker-npm-ci] npm $display_npm_version" >&2
