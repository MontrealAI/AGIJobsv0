...
if [ "$CURRENT_NPM_VERSION" != "$COREPACK_NPM_VERSION" ]; then
  if npm install -g --no-progress --loglevel=error "npm@${COREPACK_NPM_VERSION}" >/dev/null 2>&1; then
    ...
    echo "[npm-ci] WARNING: npm install -g npm@${COREPACK_NPM_VERSION} failed; continuing with npm ${CURRENT_NPM_VERSION:-unknown}" >&2
  fi
fi

if [ "$CURRENT_NPM_VERSION" != "$COREPACK_NPM_VERSION" ]; then
  echo "[npm-ci] WARNING: Unable to activate npm@${COREPACK_NPM_VERSION}; continuing with npm ${CURRENT_NPM_VERSION:-unknown}" >&2
fi
