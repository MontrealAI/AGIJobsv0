...
if [ -n "$TARGET_NPM_VERSION" ]; then
  if command -v corepack >/dev/null 2>&1; then
    if ! corepack enable >/dev/null 2>&1; then
      echo "[npm-ci] WARNING: Failed to enable corepack; continuing with npm ${CURRENT_NPM_VERSION:-unknown}" >&2
    elif corepack prepare "npm@${TARGET_NPM_VERSION}" --activate >/dev/null 2>&1; then
      CURRENT_NPM_VERSION="$(npm -v 2>/dev/null || true)"
    else
      echo "[npm-ci] WARNING: corepack prepare npm@${TARGET_NPM_VERSION} failed; continuing with npm ${CURRENT_NPM_VERSION:-unknown}" >&2
    fi
  fi

  if [ "$CURRENT_NPM_VERSION" != "$TARGET_NPM_VERSION" ]; then
    if command -v npx >/dev/null 2>&1; then
      echo "[npm-ci] activating npm@${TARGET_NPM_VERSION} via npx fallback" >&2
      NPM_COMMAND=(npx --yes "npm@${TARGET_NPM_VERSION}")
      CURRENT_NPM_VERSION="$("${NPM_COMMAND[@]}" -v 2>/dev/null || true)"
    else
      echo "[npm-ci] WARNING: Unable to locate npx for npm@${TARGET_NPM_VERSION}; continuing with npm ${CURRENT_NPM_VERSION:-unknown}" >&2
    fi
  fi
fi
