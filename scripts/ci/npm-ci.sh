...
SYSTEM_NPM_VERSION="$(npm -v 2>/dev/null || true)"
CURRENT_NPM_VERSION="$SYSTEM_NPM_VERSION"
...
if ! "${NPM_COMMAND[@]}" "${RUN_ARGS[@]}"; then
  status=$?
  if [ "${NPM_COMMAND[0]}" != "npm" ]; then
    fallback_version="${SYSTEM_NPM_VERSION:-unknown}"
    echo "[npm-ci] npm ci failed with ${CURRENT_NPM_VERSION:-unknown}; retrying with system npm ${fallback_version}" >&2
    if ! npm "${RUN_ARGS[@]}"; then
      status=$?
      popd >/dev/null
      exit $status
    else
