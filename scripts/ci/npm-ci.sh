  ...
  if [ -n "$CURRENT_NPM_VERSION" ] && [ "$CURRENT_NPM_VERSION" != "$TARGET_NPM_VERSION" ]; then
    if ! corepack enable >/dev/null 2>&1; then
      ...
      if corepack prepare "npm@${TARGET_NPM_VERSION}" --activate >/dev/null 2>&1; then
        ...
        echo "[npm-ci] WARNING: corepack prepare npm@${TARGET_NPM_VERSION} failed; continuing with npm $CURRENT_NPM_VERSION" >&2
      fi
    fi
  fi
fi
...
if [ -n "$CURRENT_NPM_VERSION" ]; then
  if ! node "$SCRIPT_DIR/version-check.mjs" "$CURRENT_NPM_VERSION" "$TARGET_NPM_VERSION"; then
    if command -v npx >/dev/null 2>&1; then
      echo "[npm-ci] upgrading npm via npx npm@${TARGET_NPM_VERSION}" >&2
      NPM_BIN=(npx --yes "npm@${TARGET_NPM_VERSION}")
      CURRENT_NPM_VERSION="$(${NPM_BIN[@]} -v 2>/dev/null || echo "$CURRENT_NPM_VERSION")"
    else
      echo "[npm-ci] ERROR: npm $CURRENT_NPM_VERSION is older than required $TARGET_NPM_VERSION and npx is unavailable" >&2
      exit 1
    fi
  fi
else
  echo "[npm-ci] WARNING: unable to determine npm version; continuing with default npm" >&2
fi
...
FINAL_NPM_VERSION="$(${NPM_BIN[@]} -v 2>/dev/null || echo 'unknown')"
echo "[npm-ci] npm $FINAL_NPM_VERSION" >&2
echo "[npm-ci] installing in $WORKDIR" >&2
echo "[npm-ci] lockfile: $LOCKFILE_PATH" >&2

pushd "$WORKDIR" >/dev/null
"${NPM_BIN[@]}" ci "${FILTERED_ARGS[@]}"
popd >/dev/null
