...
TMP_LOG="$(mktemp)"
set +e
"${NPM_COMMAND[@]}" ci "${FILTERED_ARGS[@]}" 2> >(tee -a "$TMP_LOG" >&2)
STATUS=$?
set -e

if [ "$STATUS" -ne 0 ]; then
  if grep -q "The \`npm ci\` command can only install with an existing package-lock.json" "$TMP_LOG"; then
    FALLBACK_NPM_VERSION="$(command -v npm >/dev/null 2>&1 && npm -v 2>/dev/null || printf 'unknown')"
    echo "[npm-ci] WARNING: npm ${CURRENT_NPM_VERSION} could not detect the lockfile; retrying with system npm ${FALLBACK_NPM_VERSION}" >&2
    set +e
    npm ci "${FILTERED_ARGS[@]}" 2> >(tee -a "$TMP_LOG" >&2)
    STATUS=$?
    set -e
  fi
fi
