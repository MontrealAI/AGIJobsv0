...
describe('JobRegistry payout snapshot', function () {
  ...
  beforeEach(async () => {
    ...
    await token.connect(owner).approve(await stakeManager.getAddress(), ethers.parseEther('1000'));
    await stakeManager.connect(owner).fundOperatorRewardPool(ethers.parseEther('1000'));
  ...
  it('uses snapshot when NFT transferred after assignment', async () => {
    ...
    await registry.connect(agent).submit(jobId, ethers.id('res'), 'res', '', []);
    await validation.setResult(true);
    await validation.finalize(jobId);

    await registry.connect(employer).finalize(jobId);

    expect(await token.balanceOf(agent.address)).to.equal((reward * 150n) / 100n);
  });
